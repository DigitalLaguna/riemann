...
# spot check for paper kappa (K=0)
print("paper kappa (K=0), margin = LHS-RHS:")
for eta_frac in [1.0, 0.75, 0.5, 0.25, 0.1]:
    eta = eta_frac*eta0
    lo = max(sigma0, 1-eta); hi = 1-mu0*eta
    for sf in [0.0, 0.5, 1.0]:
        sigma = lo + sf*(hi-lo)
        mg, mu = margin(sigma, eta, kap0)
        print("  eta=%.4f sigma=%.7f mu=%.5f  margin=%.6e"%(eta, sigma, mu, mg))
