# HANDOFF — session 2026-09-16 ~19:35 UTC (tick 240)
# track: C (#55 promotion) + D (job status) | gate: all tracks OPEN (21/21 seeds)
...
## Last work
Tick 240: promoted NOTE #55 -> NUMERIC (checker PASS, RC=0). Downstream
zero-free-region constant: A_final (#40) = 0.420483467794 > A0_max (#54) =
0.392113247395366294, so A0_max binds; effective = A0_max; headline
1/A0_max = 2.55028364035787 (1.90551x the paper's 4.8596). Evidence:
evidence/2026-09-16-zfree-downstream/ (re-run byte-identical). Also: both D
jobs still running (mertens promote 3h03m CPU, promote-run.txt empty;
zero-scan 140000/999990, recent rate 3.03/s).
...
(a) TRACK D: mertens-1e12-promote.service ETA ~09-17 04:00-06:00Z. When done:
    ...
    DONE rc=0" + #39 promoted); ledger +1 NUMERIC (35).
(b) TRACK D: zero-scan-1e5.service 140000/999990; recent rate 3.03/s
    (5000 steps in last 1648s; overall 13.26/s since 16:25Z restart);
    ETA ~09-20. On completion: zero list in full-run.txt; compute S(t)
    records per zero-spacing-design.md (NOTE-level, mpmath).
(c) TRACK C: A0_max PINNED (#54) + downstream constant NUMERIC (#55:
    1/A0_max = 2.55028364035787). Next C step: owner reports
    2.55028364035787 to ANT network, or pick a new BTY constant to re-optimize.
(d) OWNER: open PR via
...
04:00-06:00Z); in flight: mertens-1e12 promotion, zero-scan ETA ~09-20
(recent rate 3.03/s, declining with t).
