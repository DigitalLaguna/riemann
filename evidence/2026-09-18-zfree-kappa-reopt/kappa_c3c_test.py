...
def w(u):
    ...
    inner = (sec2*(theta*cot - u/2)*mp.cos(u*tan) + (2*theta*cot - u)
             + mp.sin(2*theta - u*tan)*csc2
             - 2*(1 + mp.sin(theta - u*tan)/mp.sin(theta)))
    return sec2*inner
w0 = w(0)
print("w(0) computed =", mp.nstr(w0,12), " paper 5.672787598  match:", abs(w0-mp.mpf('5.672787598'))<mp.mpf('1e-6'))
def W(z):
    return mp.quad(lambda u: mp.exp(-z*u)*w(u), [0, u_max])
print("W(0) =", mp.nstr(W(0),10), " (integral of w, should be positive)")
...
N=40
worst_K=mp.mpf('inf'); worst=None; min_margin=mp.mpf('inf'); min_margin_pt=None
for i in range(N):
    if time.time()-t0>300: print("TIME GUARD hit"); break
