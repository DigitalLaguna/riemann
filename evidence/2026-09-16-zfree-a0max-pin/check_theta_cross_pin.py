# Pinned checker: A0_max to ~18 digits (track C, BTY-2026 exact m=0 term).
# Supersedes #51 (width 4.12e-11): same verification, bisection 30 -> 46 iters.
#
...
t0 = time.time()
# --- re-bisect the crossing: 30 dps, 46 iterations (width -> 0.0443/2^46 ~ 6.3e-16) ---
mp.mp.dps = 30
...
width = hi - lo
mid = (lo+hi)/2
print(f"bisection (30dps, 46 iters): lo={mp.nstr(lo,18)} hi={mp.nstr(hi,18)} width={mp.nstr(width,3)}")
print(f"consistency: new hi <= old #51 hi 0.376216048464972? {hi <= mp.mpf('0.376216048464972')}")
...
print(f"RIGOROUS: A0_max >= A0_g(hi) = {mp.nstr(A0_hi,18)}   (>= 0.392113247328? {ok_target}; old #51: 0.3921132473949603)")
print(f"ESTIMATE: A0_max ~ A0_g(mid) = {mp.nstr(A0_mid,18)}   (1/A0 = {mp.nstr(1/A0_mid,15)})")
ok = ok and ok_target
